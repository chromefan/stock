...
class StockModel extends RelationModel {
	...
	// 股票实时数据
	public function realTimeData($code) {
		...
		if (! empty ( $res [1] )) {
			$data = explode ( ',', $res [1] );
			$new_data ['price'] = $data [26]; // 当前价
			$new_data ['change'] = $data [27]; // 当日涨幅
			$new_data ['chg'] = $data [28]; // 当日涨跌幅%
		}
		return $new_data;
	}
	// 更新市值
	public function updateCap($code) {
		$map ['code'] = $code;
		$stock = $this->where ( $map )->find ();
		if (substr ( $code, 0, 1 ) == '6') {
			$code_type = '1';
		...
		if (! empty ( $res [1] )) {
			...
			if ($stock ['rev'] * 10000 == 0 || $save_data ['cap'] * 10000 == 0) {
				$save_data ['status'] = 0;
			} else {
				$save_data ['psr'] = $save_data ['cap'] / $stock ['rev'];
			}
	...
	// 个股涨幅数据
	public function changeStockList($limit = 10,$order='desc') {
	...
	// 热门股票
	public function hotStockList($limit = 10){
		...
		if (empty ( $data )) {
			...
			$map ['status'] = 1;
			$data =  $this->where($map)->order ( 'chg10 desc' )->limit ( $limit )->select ();
			// S('bestStockList_'.$limit,$data,C('CACHE_TIME'));
		}
		return $data;		
	}
	//10日跌幅榜股票
	public function DayChgStockList($limit = 10,$order='desc'){
		// $data = S('bestStockList_'.$limit);
		$data=array();
		if (empty ( $data )) {
			$map ['price'] = array (
			...
			$map ['status'] = 1;
			$data =  $this->where($map)->order ( 'chg10 '.$order )->limit ( $limit )->select ();
			// S('bestStockList_'.$limit,$data,C('CACHE_TIME'));
		}
		return $data;		
	}
	...
	public function topStockList($limit = 10) {
		...
		if (empty ( $data )) {
			...
			$map ['status'] = 1;
			/*
			 * $data = $this->where($map)->order('roi_mid desc,roi desc ')->limit(100)->select(); foreach($data as $k =>$v){ $this->value($v['code']); }
			 */
	...
	public function value($code) {
		...
		/*
		 * if(empty($res) && $all==false){ D('Stock')->curl_get_data($code); redirect('/'.$code.'.html'); }
		 */
