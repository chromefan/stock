...
class Image {
     ...
     * @param int $location  添加水印的位置 1左上 2右上 3右下 4左下
     * @param string $alpha  水印的透明度
      +----------------------------------------------------------
     * @return void
      +----------------------------------------------------------
     */
	static public function water($source, $water, $savename=null, $location='3', $alpha=80) {
		//检查文件是否存在
		...
		if ($info !== false) {
			...
			if ($type == 'png' || $type == 'gif') {
				$imageFun($thumbImg, $thumbname);
			     ...
			     $imageFun($thumbImg, $thumbname, 75);
			}
	...
	static function thumbSize($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true,$mobile=true) {
		...
		if ($info !== false) {
			...
			if ($scale >= 1) {
				...
				// 缩略图尺寸
				$scale1 = $maxWidth / $srcWidth; // 计算缩放比例
				$scale2 = $maxHeight / $srcHeight;


				$scale  = $mobile?$scale2:$scale1;
	...
	static function thumbCenter($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true,$mobile=true) {

		$image = self::thumbSize($image, $thumbname, $type, $maxWidth, $maxHeight, $interlace,$mobile);//先进入等比例缩放,高度优先
		...
		if($image)
		{
			...
			if ($info !== false) {
				...
				if ($scale >= 1) {
					// 超过原图大小不再缩略
					$x_width = $width = $srcWidth;
					$y_height = $height = $srcHeight;
					$x = 0;
					$y = 0;
				} else {
					// 缩略图尺寸,否则从原图像中间开始切
					...
					{
						$x = ceil(($srcWidth-$maxWidth)/2);//开始剪裁的X坐标
						$y = 0;
						$x_width = $maxWidth+$x;
						$y_height = $maxHeight;
					}else
					{
						$y = ceil(($srcHeight-$maxHeight)/2);//开始剪裁的Y坐标
						$x = 0;

						$x_width = $maxWidth;
						$y_height = $maxHeight+$y;
					}
				...
				// 复制图片
				/*if (function_exists("ImageCopyResampled"))
				imagecopyresampled($thumbImg, $srcImg, 0, 0, $x, $y, $x_width, $y_height, $srcWidth, $srcHeight);
				else
				imagecopyresized($thumbImg, $srcImg, 0, 0, $x, $y, $x_width, $y_height, $srcWidth, $srcHeight);
				*/

				imagecopy($thumbImg, $srcImg, 0, 0, $x, $y, $x_width, $y_height);//不改变原图比例的情况下缩
	...
	static function thumbCenterBak($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
