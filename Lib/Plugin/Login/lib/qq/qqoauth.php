...
class OAuth {
	...
	public static function getOpenid($access_token) {
		...
		$url = self::$openidURL . '?' . http_build_query ( $params );
		$content = file_get_contents ( $url );
		preg_match ( '/callback\(\s+(.*?)\s+\)/i', $content, $aTemp );
...
class Tencent {
	// 接口url
	public static $apiUrlHttp = 'http://graph.qq.com/';
	...
	public static function api($command, $params = array(), $method = 'GET', $multi = false) {
		if (isset ( $_SESSION ['q_access_token'] )) { // OAuth 2.0 方式
		                                              // 鉴权参数
			$params ['access_token'] = $_SESSION ['q_access_token'];
			$params ['oauth_consumer_key'] = OAuth::$client_id;
			$params ['openid'] = $_SESSION ['q_openid'];
			$params ['format'] = 'json';
...
class Http {
	...
	public static function request($url, $params = array(), $method = 'GET', $multi = false, $extheaders = array()) {
		...
		curl_setopt ( $ci, CURLOPT_CONNECTTIMEOUT, 30000 );
		curl_setopt ( $ci, CURLOPT_TIMEOUT, 30000 );
